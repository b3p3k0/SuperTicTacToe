...
export class AiUtils {
  ...
  static countBoardThreats(board: MiniBoardState, player: Player): number {
  ...
  static countBoardForks(board: MiniBoardState, player: Player): number {
    if (board.winner || board.isFull) {
      return 0;
    }
    let forkCount = 0;
  ...
  static countMetaThreats(boards: MiniBoardState[], player: Player): number {
    return WIN_PATTERNS.reduce((total, pattern) => {
      let playerOwned = 0;
      let opponentOwned = 0;
      let openBoards = 0;
      for (const idx of pattern) {
        const winner = boards[idx]?.winner ?? null;
        if (winner === player) {
          playerOwned += 1;
  ...
  static countCenterControl(board: MiniBoardState, player: Player): number {
  ...
  static estimateBattleStability(board: MiniBoardState, owner: Player): number {
  ...
  static countLineThreats(cells: CellValue[], player: Player): number {
    const opponent = this.getOpponent(player);
    let total = 0;
    for (const pattern of WIN_PATTERNS) {
      let playerMarks = 0;
